...
build_targets() {
    ...
    cd $debug_build_dir && \
    cmake \
        -DCMAKE_C_FLAGS="-O0 -g -ggdb -std=c99 -Wall -Wextra -fprofile-arcs -ftest-coverage -DFUZZ_TOOL=3" \
...
do_continue_fuzz_libfuzzer() {
    ...
    ./$binary_dir/fuzz-test/$target $output_dir/queue_all -jobs=$num_cores -dict=fuzz.dict -verbosity=0 -close_fd_mask=3
...
do_continue_fuzz_afl() {
    ...
    screen -dmS $masterFuzzer afl-fuzz -x fuzz.dict $fuzz_args -i $output_dir/queue_all -o $output_dir -M fuzzer1 -- ./$binary_dir/fuzz-test/$target
    sleep 1
...
do_dry_run() {
    find $input_dir -type f -exec sh -c "cat {} | ./$debug_build_dir/fuzz-test/$target > /dev/null && echo \"Return with 0: {}\"" \;
    find $output_dir/queue_all -type f -exec sh -c "cat {} | ./$debug_build_dir/fuzz-test/$target > /dev/null && echo \"Return with 0: {}\"" \;
